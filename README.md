      ...
      - - ✅ Private subnets for backend servers — outbound-only internet access via NAT
        - - ✅ No SSH keys required — access handled securely through SSM
          - - ✅ Fault-tolerant web tier with zero single point of failure
            ...
            2. 2. **NAT Gateways** — one per Availability Zone so private-subnet instances get outbound-only internet access
               3. 3. **IAM + EC2** — `ec2tossm` IAM role for SSM access, EC2 instances launched into the private subnets with Apache bootstrapped via user data
                  4. 4. **Load balancing** — `WebTG` target group with health checks, internet-facing `WebALB` spanning both public subnets
                     5. 5. **Auto Scaling** — `ASG` (min 2 / desired 4 / max 6) attached to `WebTG`, replacing unhealthy or terminated instances automatically
                        6. 6. **Security hardening** — `WebSG` inbound locked down to accept traffic only from `ALBSG`
                          
                           7. ---
                          
                           8. ## 🛠️ Bootstrapping Script (User Data)
                          
                           9. Each instance is configured automatically on launch, installing and starting an Apache web server. Full script: [`scripts/user-data.sh`](scripts/user-data.sh)
